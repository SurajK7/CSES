...
/*
diameter is a path
every path in a rooted tree has a highest point
let's calculate for each node the length of longest path with highest point as that node
diameter will be maximum of all the maximum path lengths for all nodes
*/
...
void solve()
{
    ...
    auto dfs = [&](auto &self, ll v, ll p) -> void
    {
        ...
        ckmax(res, maxx + smaxx + min(cnt, 2L));
    };
    dfs(dfs, 0, -1);
    cout << res << '\n';
}
