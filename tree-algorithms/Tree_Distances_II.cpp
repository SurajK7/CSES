...
void solve()
{
    ...
    vector<ll> dist_subtree(n), count_elem_subtree(n, 1), dist_total(n);
    auto dfs1 = [&](auto &self, ll v, ll p) -> void
    {
        for (auto i : adj[v])
        {
            if (i == p)
                continue;
            self(self, i, v);
            count_elem_subtree[v] += count_elem_subtree[i];
            dist_subtree[v] += dist_subtree[i] + count_elem_subtree[i];
        }
    };
    dfs1(dfs1, 0, -1);
    dist_total[0] = dist_subtree[0];
    auto dfs2 = [&](auto &self, ll v, ll p) -> void
    {
        for (auto i : adj[v])
        {
            if (i == p)
                continue;
            dist_total[i] = dist_total[v] + n - 2 * count_elem_subtree[i];
            self(self, i, v);
        }
    };
    dfs2(dfs2, 0, -1);
    for (auto i : dist_total)
        cout << i << ' ';
    cout << '\n';
}
